# evidence_creator.py — Create Evidence entities for each mapped control (ontology/models). POLICY_DOCUMENT, content_hash, obligations_satisfied.
...
def create_evidence_from_mapping(
    ...
    """
    For each mapped control create an Evidence entity.
    evidence_type = POLICY_DOCUMENT; content_hash; link obligations_satisfied for cross-framework coverage.
    """
    ...
    for i, ctrl in enumerate(mapping.controls):
        ...
        audit_fabric.log(
            ...
            entity_id=ev_id,
            payload={"content_hash": digest[:16], "obligations_count": len(obligation_ids)},
        )
